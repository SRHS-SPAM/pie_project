...
function _page($$payload, $$props) {
  ...
  if (stories[herringId - 1]) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<button class="button svelte-41v8yq"><img${attr("alt", `${herringId}번이미지`)}${attr("src", stories[herringId - 1].tu)} class="tu svelte-41v8yq"></button> <div><img${attr("alt", `${herringId}번스토리`)}${attr("src", stories[herringId - 1].img)} class="story svelte-41v8yq"></div> <div class="piebox svelte-41v8yq"><img${attr("alt", `${herringId}파이`)}${attr("src", stories[herringId - 1].pieimg)} class="pie svelte-41v8yq"> <img${attr("alt", `${herringId}파이`)}${attr("src", stories[herringId - 1].pieimg)} class="pie svelte-41v8yq"> <img${attr("alt", `${herringId}파이`)}${attr("src", stories[herringId - 1].pieimg)} class="pie svelte-41v8yq"></div> <div class="textbox svelte-41v8yq"><p class="simtext svelte-41v8yq">${escape_html(stories[herringId - 1].simtext)}</p> <p class="textmal svelte-41v8yq">${escape_html(stories[herringId - 1].text.replace(/\n/g, "<br>"))}</p></div>`;
  } else {
    $$payload.out += "<!--[!-->";
