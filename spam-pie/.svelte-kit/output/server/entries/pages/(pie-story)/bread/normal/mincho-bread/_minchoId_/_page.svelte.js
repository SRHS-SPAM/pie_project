...
function _page($$payload, $$props) {
  ...
  if (stories[minchoId - 1]) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<button class="button svelte-3abuuu"><img${attr("alt", `${minchoId}번이미지`)}${attr("src", stories[minchoId - 1].tu)} class="tu svelte-3abuuu"></button> <div><img${attr("alt", `${minchoId}번스토리`)}${attr("src", stories[minchoId - 1].img)} class="story svelte-3abuuu"></div> <div class="piebox svelte-3abuuu"><img${attr("alt", `${minchoId}파이`)}${attr("src", stories[minchoId - 1].pieimg)} class="pie svelte-3abuuu"> <img${attr("alt", `${minchoId}파이`)}${attr("src", stories[minchoId - 1].pieimg)} class="pie svelte-3abuuu"> <img${attr("alt", `${minchoId}파이`)}${attr("src", stories[minchoId - 1].pieimg)} class="pie svelte-3abuuu"></div> <div class="textbox svelte-3abuuu"><p class="simtext svelte-3abuuu">${escape_html(stories[minchoId - 1].simtext)}</p> <p class="textmal svelte-3abuuu">${escape_html(stories[minchoId - 1].text.replace(/\n/g, "<br>"))}</p></div>`;
  } else {
    $$payload.out += "<!--[!-->";
